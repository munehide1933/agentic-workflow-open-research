...
次のいずれかを満たすと `second_pass_eligible=true`：

1. `risk_level=high`
2. `intent_type in {diagnosis, codegen, ops}`
3. `requires_executable=true`

低リスクで実行可能出力を要求しない `qa` / `architecture` は既定で `false`。
...
| `S3_DRAFT_READY` | `S4_AUDIT_READY` | `second_pass_eligible=true` | 決定論的 eligibility ガード一致 |
