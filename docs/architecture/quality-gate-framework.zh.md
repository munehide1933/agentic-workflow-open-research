...
## 4. `semantic_safety_check` 操作化方法

`semantic_safety_check` 由“确定性规则 + 可选模型复核”组成：

1. 规则匹配（必选）
: 检查禁止操作、前置条件缺失、权限/作用域越界。
2. 意图一致性检查（必选）
: 检查生成动作是否超出用户声明意图。
3. 复核模型检查（可选）
: 第二模型评估隐性风险；不可用时允许仅规则路径。
...
2. 任一 `semantic_findings.severity=critical` => `R3`
3. `intent_drift=true` 且无 critical 时，至少为 `R2`
4. 最大风险为 `R0-R1` => `pass`
5. 最大风险为 `R2` => `soft_fail`
6. 最大风险为 `R3` => `hard_fail`
...
- `semantic_findings`：语义安全发现列表
- `remediation`：安全替代与验证步骤

## 8. 与 Anchor Guard 的关系
...
1. 语法通过且无危险模式且无语义发现 => `pass`。
2. 语法通过且中风险模式 => `soft_fail`。
3. 意图漂移但无 critical => 至少 `soft_fail`。
4. critical 语义违规 => `hard_fail`。
5. Anchor Guard 阻断 + Quality Gate 通过 => 最终仍按阻断处理。
