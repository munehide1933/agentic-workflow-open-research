...
满足任一条件则 `second_pass_eligible=true`：

1. `risk_level=high`
2. `intent_type in {diagnosis, codegen, ops}`
3. `requires_executable=true`

默认 `second_pass_eligible=false`：低风险且无可执行输出需求的 `qa` 与 `architecture` 请求。
...
| `S3_DRAFT_READY` | `S4_AUDIT_READY` | `second_pass_eligible=true` | 命中确定性 eligibility 守卫 |
| `S3_DRAFT_READY` | `S5_FINAL_READY` | `second_pass_eligible=false` | 直接 finalize |
