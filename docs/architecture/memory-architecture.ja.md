...
公開デフォルト `min_score=0.72` は、既定 embedding/index 構成のコサイン類似度を前提に校正した値である。

embedding モデル、距離指標、または index 設定を変更する場合は再校正を行うこと。

1. ラベル付き検証セット（最低 200 query-memory ペア）を準備。
2. `[0.50, 0.90]` を `0.02` 刻みで閾値走査。
3. 目的関数 `J = 0.7*precision + 0.3*recall` を計算。
4. `J` 最大かつ誤注入許容範囲内の閾値を採用。
5. benchmark メタデータに新閾値とデータセット要約を公開。

この再校正なしでは、実装間で `min_score` 比較可能性は保証されない。

## 7. Verification-First との連携
...
7. backend 変更時に再校正で `0.72` 代替閾値を提示できる。
8. ロールバック時に直近 commit 状態へ復元される。
