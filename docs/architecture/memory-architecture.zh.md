...
公开默认值 `min_score=0.72` 针对默认 embedding/index 配置下的余弦相似度校准得到。

若替换 embedding 模型、距离度量或索引配置，需执行再校准：

1. 准备标注检索验证集（至少 200 条 query-memory 对）。
2. 在 `[0.50, 0.90]` 区间按步长 `0.02` 扫描阈值。
3. 计算目标函数 `J = 0.7*precision + 0.3*recall`。
4. 选择最大化 `J` 且注入误报可接受的阈值。
5. 在 benchmark 元数据中公开新阈值与数据集摘要。

未执行该再校准时，不保证不同实现间 `min_score` 的可比性。

## 7. 与 Verification-First 的绑定
