...
### 3.3 Second-Pass Timeout Profile 連携

second-pass ガバナンスは解決済み timeout profile を metadata で公開する。現行 runtime の出力：

- `level`: `low | medium | high | extreme`
- `score`: 正規化複雑度スコア（`0..1`）
- `base_seconds` / `resolved_seconds` / `max_seconds`
- `factors[]`: 短い説明文字列（例 `token:1850`, `domain:DISTRIBUTED+0.20`）
...
8. 高複雑度 second-pass 要求：
   - 期待：timeout profile は `high` または `extreme` に解決され、factors を含む。
