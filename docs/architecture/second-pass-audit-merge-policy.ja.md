...
- `timeout_profile`: second-pass の解決済み timeout profile metadata
...
2. v1 は読み取り互換のみ維持する。
3. この発効日は本公開研究リリース系統での最終日付です。
4. 空 `counter_hypotheses` を伴う partial audit は v2 を必須とする。
5. v1 で空 `counter_hypotheses` は schema 不正となり `invalid` 扱い。

### 3.3 Second-Pass Timeout Profile 契約（v1）

runtime が公開する解決済み timeout metadata フィールド：
...
契約 schema: [`examples/contracts/second-pass-timeout-profile.schema.v1.json`](../../examples/contracts/second-pass-timeout-profile.schema.v1.json)
...
`draft`、`challenge`、`rules` が同一なら、`merged` も同一でなければならない。

### 6.5 ユーザー面表示モード契約

現行 runtime は 3 つのユーザー面表示モードを持つ：

- `hidden`（既定）：監査ブロックをユーザー回答に表示しない
- `summary`：短いレビュー注記のみ表示
- `full`：監査が信頼可能な場合のみ監査ブロックを表示

safe patch が有効で監査が信頼可能な場合、runtime は最小パッチ行のみ追加し、draft 本文を先頭として維持する。
...
7. timeout profile が上限到達（`resolved_seconds == max_seconds`）=> マージ経路は上限予算を遵守
8. 同一 `draft + challenge + rules` を replay => merged 結果は完全一致
